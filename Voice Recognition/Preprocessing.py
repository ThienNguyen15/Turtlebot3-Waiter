...
def extract_table_from_ordered_text(text):
    ...
    match = re.search(r'\bin table (\d+)\b', text)
    if match:
...
def table_command_to_database(table_id):
    ...
    db.collection("actions").document(str(table_data["command_id"])).set(table_data)
    ...
    while True:
        ...
        if user_input == "e":
            ...
            if order_items:
                ...
                table_id = extract_table_from_ordered_text(clean_transcribed_text)
